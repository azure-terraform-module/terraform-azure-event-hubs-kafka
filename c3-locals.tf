locals {
  ...
  private_dns_zone_ids = local.is_private ? (
    var.eventhub_private_dns_zone_id != null ? [var.eventhub_private_dns_zone_id] : (
  ...
  network_rulesets = [
    {
      default_action                  = local.is_public ? "Allow" : "Deny"
      trusted_service_access_enabled = true
      public_network_access_enabled  = local.is_service || local.is_public
      ...
      # IP rules - Service endpoints, Public endpoints
      ip_rule = [
        for ip in var.ip_rules : {
          ip_mask = ip
          action  = "Allow"
        } if (local.is_service || local.is_public)
      ]
  ...
  public_network_access = local.is_private ? false : true
}
